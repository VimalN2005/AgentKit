...
## Guardrails
- **Credential Protection**: Automatically suppresses and redacts Notion integration tokens (`secret_...`), Bearer tokens, and private API keys.
- **PII Redaction**: Redacts phone numbers, personal email addresses, and private contact information from generated blocks.
- **Schema Validation**: Ensures all generated child blocks strictly follow Notion API block types (`heading_1`, `paragraph`, `to_do`, `bulleted_list_item`, `callout`, `code`).

## Integration Reference
- **Notion API**: Interacts with the Notion REST API (`/v1/pages`, `/v1/databases`, `/v1/search`, `/v1/blocks/{id}/children`).
- **Lamatic Studio**: Orchestrates the LLM prompt execution, schema parsing, and API response mapping.

## Environment Setup
No external environment variables are required for basic flow execution. When deploying with live Notion API integration:
...
2. Send a POST request to the flow endpoint with a JSON body:
   ```json
   {
